...
class HailoDetectionCallback(app_callback_class):
    ...
    def get_fallback_pipeline_string(self):
        ...
        return "v4l2src device=/dev/video2 name=source_0 ! image/jpeg, width=640, height=360, framerate=25/1 ! jpegdec ! videoconvert ! video/x-raw, format=RGB, pixel-aspect-ratio=1/1 ! queue name=inference_scale_q leaky=downstream max-size-buffers=10 ! videoscale name=inference_videoscale n-threads=2 qos=false ! queue name=inference_convert_q leaky=downstream max-size-buffers=10 ! videoconvert name=inference_videoconvert n-threads=2 ! video/x-raw, pixel-aspect-ratio=1/1 ! queue name=inference_hailonet_q leaky=downstream max-size-buffers=10 ! hailonet name=inference_hailonet hef-path=resources/ai_model.hef batch-size=1 vdevice-group-id=1 nms-score-threshold=0.3 nms-iou-threshold=0.45 output-format-type=HAILO_FORMAT_TYPE_FLOAT32 force-writable=false ! queue name=inference_hailofilter_q leaky=downstream max-size-buffers=10 ! hailofilter name=inference_hailofilter so-path=/home/afiq/hailo-rpi5-examples/resources/libyolo_hailortpp_postprocess.so config-path=resources/labels.json function-name=filter_letterbox qos=false ! queue name=inference_output_q leaky=downstream max-size-buffers=10 ! hailotracker name=hailo_tracker class-id=-1 kalman-dist-thr=0.8 iou-thr=0.9 init-iou-thr=0.7 keep-new-frames=1 keep-tracked-frames=1 keep-lost-frames=1 keep-past-metadata=false qos=false ! queue name=hailo_tracker_q leaky=downstream max-size-buffers=10 ! identity name=identity_callback_0 ! queue name=hailo_display_hailooverlay_q leaky=downstream max-size-buffers=10 ! hailooverlay name=hailo_display_hailooverlay ! queue name=hailo_display_videoconvert_q leaky=downstream max-size-buffers=10 ! videoconvert name=hailo_display_videoconvert n-threads=2 qos=false ! queue name=hailo_display_q leaky=downstream max-size-buffers=10 ! fpsdisplaysink name=hailo_display video-sink=xvimagesink sync=false text-overlay=false signal-fps-measurements=true"
...
tracker = EnhancedTracker(max_age=1, min_hits=1, iou_threshold=0.3, max_disappeared=1)
